...
namespace KerbalEngineDynamics
{
    ...
    public class EngineMasteryTracker : ScenarioModule
    {
        ...
        private void OnVesselRecovered(ProtoVessel protoVessel, bool quick)
        {
            foreach (ProtoPartSnapshot pps in protoVessel.protoPartSnapshots)
            {
                ...
                {
                    if (ppms.moduleName == "KEDModule")
                    {
                        float burnSec = 0f;
                        bool failed = false;
                        if (ppms.moduleValues.TryGetValue("cumulativeBurnSeconds", ref burnSec) && burnSec >= 15f)
                        {
    ...
    public class KEDModule : PartModule, IModuleInfo
    {
        ...
        public void FixedUpdate()
        {
            ...
            if (isFailed)
            {
                if (failureType == 3) { engine.Shutdown(); engine.allowRestart = false; engine.Events["Activate"].active = false; }
                return;
            }
        ...
        private void HandleIgnitionEvent()
        {
            ...
            if (currentArchetype == EngineArchetype.Thermodynamic)
            {
                ...
                float baseRisk = (block == 0) ? 0.05f * 2.0f : 0.05f;
                float finalRisk = Mathf.Clamp(baseRisk + ignitionFatigue, 0f, (block >= 3) ? 0.02f : 1.0f);
        ...
        private void HandleRunningEvent()
        {
            ...
            if (currentArchetype == EngineArchetype.Solid)
            {
                ...
                {
                    float risk = (block >= 6) ? 0.005f : (block >= 2) ? 0.075f : 0.10f; // 50% marker peak
                    if (stages[srbStagePassed] != 0.5f) risk *= 0.5f; 
                    if (block == 0) risk *= 2.0f;

                    if (UnityEngine.Random.value < risk) SetFailureState(false, 3); // Casing Breach
                    srbStagePassed++;
                }
            }
            // Thermodynamic Interval Check (10s)
            else if (currentArchetype == EngineArchetype.Thermodynamic && checkTimer >= 10f)
            {
                checkTimer = 0f;
            ...
            else if (currentArchetype == EngineArchetype.Monopropellant && checkTimer >= 5f)
            {
                ...
                if (cumulativeBurnSeconds > limit)
                {
                    ...
                    if (UnityEngine.Random.value < risk) SetFailureState(false, 3); // Catalyst Choke
                }
            }
        }
        ...
        private void SetFailureState(bool silent, int mode)
        {
            ...
            Events["PerformEVAAction"].active = true;
        ...
        public void HardwareRetrofit()
        {
            ...
            if (inv != null && inv.ContainsPart("evaRepairKit", kitsNeeded)) 
            {
        ...
        {
            if (currentArchetype == EngineArchetype.Thermodynamic) {
                if (fType == 2) return (lvl >= 2) ? 1 : 2; // Ignition
                if (fType == 3) return (lvl >= 4) ? 1 : (lvl >= 2) ? 2 : 4; // Overhaul
                return (lvl >= 2) ? 1 : 2; // Gimbal
            }
            if (currentArchetype == EngineArchetype.Monopropellant) return (lvl >= 4) ? 1 : (lvl >= 2) ? 2 : 3; // Flush
            if (currentArchetype == EngineArchetype.Solid) return (lvl >= 4) ? 2 : (lvl >= 2) ? 4 : 6; // Patch
            return 8; 
        }

        private void ExecuteRepair()
        {
            isFailed = false; failureType = 0; wasBurning = false; 
        ...
        public void HarvestTelemetry()
        {
            ...
            Vessel v = FlightGlobals.ActiveVessel;
            if (v == null || !v.isEVA || v.GetVesselCrew()[0].experienceTrait.Title != "Engineer") return;
        ...
        private void RefreshLiveUI()
        {
            ...
            uiCertLevel = $"Block {(displayBlock == 0 ? "X-0" : displayBlock == 1 ? "I" : displayBlock == 2 ? "II" : displayBlock == 3 ? "III" : displayBlock == 4 ? "IV" : displayBlock == 5 ? "V" : "VI")}";
            ...
            if (currentArchetype == EngineArchetype.Solid) uiArchetypeStatus = $"Casing Stress: {(cumulativeBurnSeconds < 10f ? "Safe" : "Active")}";
            else if (currentArchetype == EngineArchetype.Monopropellant) uiArchetypeStatus = $"Catalyst: {cumulativeBurnSeconds:F1}s Active";
            else uiArchetypeStatus = $"Fatigue: {(ignitionFatigue * 100):F1}%";

            if (isFailed) uiArchetypeStatus = $"FAILED: {GetFailureName(failureType)}";
            
            if (currentArchetype == EngineArchetype.Thermodynamic) Events["PerformEVAAction"].guiName = "Perform Systems Overhaul";
            else if (currentArchetype == EngineArchetype.Monopropellant) Events["PerformEVAAction"].guiName = "Perform Catalyst Flush";
            else Events["PerformEVAAction"].guiName = "Apply Structural Patch";
        ...
        private string GetFailureName(int m) => m == 1 ? "Gimbal Lock" : m == 2 ? "Ignition Failure" : "Engine Flameout";
